...
def _ridge_cv_age(Xs: np.ndarray, age: np.ndarray, lam: float, seed: int):
    """Out-of-fold ridge predictions of chronological age (leak-free clock)."""
    n = len(age)
    ...
    for f in folds:
        ...
        w = np.linalg.solve(Xs[tr].T @ Xs[tr] + lam * np.eye(k),
                            Xs[tr].T @ (age[tr] - age[tr].mean()))
        pred[f] = Xs[f] @ w + age[tr].mean()
    return pred


def _select_lambda(Xs: np.ndarray, age: np.ndarray, seed: int):
    """Pick ridge penalty by out-of-fold AGE-prediction R2 (outcome-blind)."""
    best = None
    for lam in LAMBDA_GRID:
        pred = _ridge_cv_age(Xs, age, lam, seed)
...
def fit_clock(df: pd.DataFrame, cfg: dict, spec: str = PRIMARY_SPEC):
    ...
    lam, r2 = _select_lambda(Xs, age, seed)
    imaging_age = _ridge_cv_age(Xs, age, lam, seed)
    ...
    # ridge coefficients on the full data (for the contribution figure)
...
def clock_summary(df: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    """Clock-fit metrics for every spec (primary + sensitivity)."""
    rows = []
...
def spec_sensitivity(df: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    """Primary association (adj age, sex, ASA) across all clock specs."""
    rows = []
    for spec in CLOCK_SPECS:
        a = aar_association(df, cfg, spec)
